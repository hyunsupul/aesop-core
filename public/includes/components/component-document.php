...
if (!function_exists('aesop_document_shortcode')){
	...
	function aesop_document_shortcode($atts, $content = null) {
		...
		$out .= sprintf('%s<aside %s class="aesop-documument-component aesop-content %s">%s%s%s%s</aside>%s',$actiontop, esc_attr( aesop_component_data_atts( 'document', $unique, $atts )), $classes, $actioninsidetop, $link, $guts, $actioninsidebottom, $actionbottom);
