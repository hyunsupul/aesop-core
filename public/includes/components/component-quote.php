...
if (!function_exists('aesop_quote_shortcode')){
	...
	function aesop_quote_shortcode($atts, $content = null) {
		...
		?>
			<div data-unique="<?php echo esc_attr( $unique );?>" id="aesop-quote-component-<?php echo esc_attr( $unique );?>" class="aesop-component aesop-quote-component <?php echo sanitize_html_class( $classes ).' '.sanitize_html_class( $align ).' '.sanitize_html_class( $contentwidth ).' '.sanitize_html_class( $isparallax ).' '.sanitize_html_class( $lrclass ).' ';?>" <?php echo $style;?>>
