...
if (!function_exists('aesop_character_shortcode')){
	...
	function aesop_character_shortcode($atts, $content = null) {
		...
		ob_start();
			...
			?>
				<aside id="aesop-character-component-<?php echo esc_attr( $unique );?>" <?php echo esc_attr( aesop_component_data_atts( 'character', $unique, $atts ) );?> class="aesop-character-component aesop-component <?php echo sanitize_html_class( $classes ).''.sanitize_html_class( $float );?> ">
