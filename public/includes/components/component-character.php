...
if (!function_exists('aesop_character_shortcode')){
	...
	function aesop_character_shortcode($atts, $content = null) {
		...
		ob_start();
			...
			?>
				<aside contenteditable="false"  data-unique="<?php echo esc_attr( $unique );?>" id="aesop-character-component-<?php echo esc_attr( $unique );?>" class="aesop-character-component <?php echo sanitize_html_class( $classes ).''.sanitize_html_class( $float );?> ">
